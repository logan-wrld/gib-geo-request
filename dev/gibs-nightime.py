...
for layer_name, layer in wms.contents.items():
    print(layer_name, layer.title)

# Get the date from yesterday
yesterday = (datetime.date.today() - datetime.timedelta(days=1)).strftime("%Y-%m-%d")

# Configure request for VIIRS Day/Night Band, adjusting to the coordinates of Texas, USA
texas_bbox = (-106.65, 25.84, -93.51, 36.5)  # Bounding box for Texas

# Request nighttime image
img_response = wms.getmap(layers=['VIIRS_SNPP_DayNightBand_ENCC'],
...
png_output_file = os.path.join(output_dir, 'VIIRS_DayNightBand_Texas.png')
...
bmp_output_file = os.path.join(output_dir, 'VIIRS_DayNightBand_Texas.bmp')
